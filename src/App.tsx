      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="absolute top-8 right-8 flex items-center gap-2 p-1.5 rounded-full border border-current opacity-40 hover:opacity-80 transition-opacity text-xs font-bold uppercase overflow-hidden">
            <button onClick={() => setLang('id')} className={`px-2 py-1 rounded-full transition-colors ${lang === 'id' ? 'bg-text-sanc text-bg-sanc' : ''}`}>ID</button>
            <button onClick={() => setLang('en')} className={`px-2 py-1 rounded-full transition-colors ${lang === 'en' ? 'bg-text-sanc text-bg-sanc' : ''}`}>EN</button>
          </div>
